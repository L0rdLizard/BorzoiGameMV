...
public class EditingView extends State implements StateMethods {
...
//        editButtons[5] = new MenuButton(imgs[1], 64, 0, 64, 64);
    ...
    public void drawMiniLevel(Graphics g){
        for (int j = 0; j < game.getEditing().getLvlData().length; j++){
            for (int i = 0; i < game.getEditing().getLvlData()[0].length; i++){
                int index = game.getEditing().getLvlData()[j][i];
//                if (index >= 48)
//                    index = 0;
                if (index == 100)
    ...
    private void clickCheck(int x, int y){
        if (x >= 160 && x <= 1760 && y >= 96 && y <= 736){
            int xIndex = (x - 160) / 32;
            int yIndex = (y - 96) / 32;
            game.getEditing().changePixel(xIndex, yIndex, game.getEditing().getLvlData(), choosenBlock);
        }
...
//        g.drawImage(imgs[i+2], (1248 ), 800 + 96 * i, 128, 64, null);
        ...
        if (x >= 1088 && x <= 1216 && y >= 800 && y <= 864){
            game.getEditing().setCurrentEditingLevel(0);
        }
        //level2
        if (x >= 1248 && x <= 1376 && y >= 800 && y <= 864){
            game.getEditing().setCurrentEditingLevel(1);
        }
        //level3
        if (x >= 1088 && x <= 1216 && y >= 896 && y <= 960){
            game.getEditing().setCurrentEditingLevel(2);
        }
        //level4
        if (x >= 1248 && x <= 1376 && y >= 896 && y <= 960){
            game.getEditing().setCurrentEditingLevel(3);
        }
    }
